...
/*   Updated: 2026/05/31 18:17:47 by mghitta          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	compile_check(t_simulation *sim,
	t_coder *coder, int *count_coder_finished_compile)
{
	...
	while (get_stop(sim) == 0)
	{
		...
		while (i < sim->number_of_coders)
		{
			if (burnout_check(sim, &sim->coders[i]) == 1)
				break ;
			if (compile_check(sim, &sim->coders[i],
					&count_coder_finished_compile) == 1)
				break ;
			i++;
		}
